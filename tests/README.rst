...
To run the regression tests from the recycle/tests:
...
.. code-block:: bash
  ...
  $ rename 's/^[^_]*_[^_]*_(.*)/<cyclus version>_<recycle version>_$1/' *.h5

where

* <cyclus version> is replaced by the current version tag name for cyclus
  (e.g. v0.1)
* <recycle version> is replaced by the current version tag name for recycle
  (e.g. v0.1)
