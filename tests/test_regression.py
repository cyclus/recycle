...
class _PhysorEnrichment(TestRegression):
    ...
    def setUp(self):
        ...
        self.rx_id = self.find_ids(":recycle:Reactor", tbl)
        self.enr_id = self.find_ids(":recycle:Enrichment", tbl)
...
class _PhysorSources(TestRegression):
    ...
    def setUp(self):
        ...
        tbl = self.agent_entry
        rx_id = self.find_ids(":recycle:Reactor", tbl)
        self.r1, self.r2, self.r3 = tuple(rx_id)
        s_id = self.find_ids(":recycle:Source", tbl)
...
class TestDynamicCapacitated(TestRegression):
    ...
    def setUp(self):
        ...
        self.source_id = self.find_ids(":recycle:Source", self.agent_entry)
        self.sink_id = self.find_ids(":recycle:Sink", self.agent_entry)
